...
void c_polylogarithmic_samp( int r, int NITE, int *X, int N, float F ){
   int k,j,samp,ite;
   float P,q,log_term1,log_term0,f_term,log_sum,sum_over_x;
   for( ite = 1 ; ite <= NITE ; ite++ ){
     ...
     for( k=0; k < N ; k++ ){
		...
		// --------------------------------------------------------------------------------
		log_term0 = (sum_over_x - ((float)X[ samp + k ])) / ( (float) N ) ;
	    if( fabs( log_term0 ) + 1.0 <= 1e-08 ){
		    printf("::c_polylogarithm_samp:: DIVISION BY ZERO log_term0...");
		    return;
	    }
	    log_term0 = log( log_term0 + 1.0  );
	    
	    log_term1 = (1.0 + (sum_over_x - ((float)X[ samp + k ])) ) / ( (float) N ) ;
	    if( fabs( log_term1 ) + 1.0 <= 1e-08 ){
		    printf("::c_polylogarithm_samp:: DIVISION BY ZERO log_term1...");
		    return;
	    }
	    log_term1 = log( log_term1 + 1.0 );
		
		f_term    = F * (log_term0 - log_term1);
		
		// Compute summation over j [ (log_term|x_k=1 - (j-1)) - (log_term|x_k=0 - (j-1)) ]
		// --------------------------------------------------------------------------------
		log_sum   = log( 1.0 + sum_over_x - ((float)X[ samp + k ]) ) ;
